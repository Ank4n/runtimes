...
use kusama_runtime_constants::{currency::UNITS, system_parachain::ASSET_HUB_ID};
...
use staging_kusama_runtime::{xcm_config::XcmConfig, PostAhmFilter, Runtime, RuntimeCall};
...
fn proxies_keep_working_but_stop_changing() {
	let add = RuntimeCall::Proxy(pallet_proxy::Call::<Runtime>::add_proxy {
		delegate: ALICE.into(),
		proxy_type: staging_kusama_runtime::TransparentProxyType(
			kusama_runtime_constants::proxy::ProxyType::Any,
		),
		delay: 0,
	});
