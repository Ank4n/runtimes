...
fn receive_mints_free_and_holds_exactly() {
	new_test_ext().execute_with(|| {
		let alice: AccountId = 1; // regular account: liquid + migrated reserve
		let charlie: AccountId = 3; // already exists locally, receives on top

		// GIVEN charlie already has a local balance.
		<Balances as Mutate<AccountId>>::mint_into(&charlie, 100).unwrap();
...
fn receive_parks_bad_account_without_poisoning_batch() {
	new_test_ext().execute_with(|| {
		let eve: AccountId = 5; // integrates fine
		let dave: AccountId = 4; // mint overflows total issuance -> must park
