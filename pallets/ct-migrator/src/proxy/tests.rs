...
fn receive_recreates_defs_and_resizes_the_deposit_to_local_rates() {
	new_test_ext().execute_with(|| {
		...
		Receiver::receive(vec![portable_proxy(
			&pure,
			vec![(controller.clone(), PortableProxyType::Any, 4)],
		)]);
...
fn delay_conversion_rounds_down() {
	new_test_ext().execute_with(|| {
		...
		Receiver::receive(vec![portable_proxy(
			&delegator,
			vec![(d1.clone(), PortableProxyType::Any, 3), (d2.clone(), PortableProxyType::Any, 1)],
		)]);
...
fn receive_merges_with_existing_local_defs_and_dedups() {
	new_test_ext().execute_with(|| {
		...
		Receiver::receive(vec![portable_proxy(
			&dan,
			vec![
				(migrated.clone(), PortableProxyType::Any, 0),
				(local.clone(), PortableProxyType::Any, 0),
			],
		)]);
...
fn receive_writes_the_entry_even_when_the_deposit_cannot_be_reserved() {
	new_test_ext().execute_with(|| {
		...
		Receiver::receive(vec![portable_proxy(
			&broke,
			vec![(delegate.clone(), PortableProxyType::NonTransfer, 0)],
		)]);
